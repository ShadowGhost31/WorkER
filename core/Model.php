...
class Model
{
    ...
    public static function deleteByUId($id){
        Core::get()->db->delete(static::$tableName, [static::$uid => $id]);
    }
    ...
    public static function findByUId($id){
        $arr = Core::get()->db->select(static::$tableName,'*', [static::$uid => $id]);
        if(count($arr) > 0){
            return $arr[0];
        } else
            return null;
    ...
    public static function findByConditionES($conditionAssocArray){
        $arr = Core::get()->db->select(static::$tableName,'*', [$conditionAssocArray]);
        if(count($arr) > 0){
            return $arr;
        } else
    ...
    {
        return Core::get()->db->selecter(static::$tableName, '*');
    }
